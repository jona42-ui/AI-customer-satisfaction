...
def model_promoter(mse: float, stage: str = "production") -> bool:
    ...
    try:
        ...
        mse_dict = previous_production_model.get_artifact("sklearn_regressor").run_metadata["metrics"]
        previous_production_model_mse = float(mse_dict["mse"]) if "mse" in mse_dict else mse + 1000
    except RuntimeError:
